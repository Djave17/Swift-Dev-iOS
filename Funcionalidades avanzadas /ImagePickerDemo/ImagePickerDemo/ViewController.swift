...
class ViewController: UIViewController {
    ...
    @objc func addMedia(){
        ...
        configuration.filter = .images //seleccionamos el filtro
        configuration.selectionLimit = 1 //limite de seleccion,  0 = no tiene limites..
...
extension ViewController: PHPickerViewControllerDelegate {
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        if itemProvider.canLoadObject(ofClass: UIImage.self){
            itemProvider.loadObject(ofClass: UIImage.self, completionHandler: { [weak self] reading, error in
                guard error == nil, let image = reading as? UIImage else {return}
                
                
                let selectedImage = MediaModel(type: .photo, image: image)
                
                self?.medias.append(selectedImage)
                self? .applySnapshot()
